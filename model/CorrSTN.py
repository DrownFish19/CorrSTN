...
import math
import numpy as np
from lib.utils import norm_Adj, multichannel_norm_adj
...
class spatialGCN(nn.Module):
    def __init__(self, sym_norm_Adj_matrix, sym_norm_sc_matrix, in_channels, out_channels):
        super(spatialGCN, self).__init__()
        self.sym_norm_Adj_matrix = sym_norm_Adj_matrix  # (N, N)
        self.sym_norm_sc_matrix = sym_norm_sc_matrix  # (C, N, N)
        C = sym_norm_sc_matrix.shape[0]
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.Theta = nn.Linear(in_channels, out_channels, bias=False)
        self.alpha = torch.nn.Parameter(torch.Tensor([0.0]), requires_grad=True)
        self.beta = torch.nn.Parameter(torch.Tensor([1.0] * C), requires_grad=True)
    ...
    def forward(self, x):
        ...
        x_gcn = self.alpha * torch.matmul(self.sym_norm_Adj_matrix, x)
        for c in range(self.sym_norm_sc_matrix.shape[0]):
            x_gcn += self.beta[c] * torch.matmul(self.sym_norm_sc_matrix[c], x)
        # x = torch.matmul(self.sym_norm_Adj_matrix, x)
        return F.relu(self.Theta(x_gcn).reshape((batch_size, num_of_timesteps, num_of_vertices, self.out_channels)).transpose(1, 2))
...
class GCN(nn.Module):
    ...
    def forward(self, x):
        ...
        for c in range(self.sym_norm_sc_matrix.shape[0]):
            x_gcn += self.beta[c] * torch.matmul(self.sym_norm_sc_matrix[c], x)
        # x = torch.matmul(self.sym_norm_Adj_matrix, x)
        return F.relu(self.Theta(x_gcn))  # (N,N)(b,N,in)->(b,N,in)->(b,N,out)
...
class spatialAttentionScaledGCN(nn.Module):
    def __init__(self, sym_norm_Adj_matrix, sym_norm_sc_matrix, in_channels, out_channels, dropout=.0):
    ...
    def forward(self, x):
        ...
        spatial_attention = spatial_attention.reshape((-1, num_of_vertices, num_of_vertices))  # (b*T, n, n)
        x_gcn = self.alpha * torch.matmul(self.sym_norm_Adj_matrix.mul(spatial_attention), x)
        for c in range(self.sym_norm_sc_matrix.shape[0]):
            x_gcn += self.beta[c] * torch.matmul(self.sym_norm_sc_matrix[c].mul(spatial_attention), x)
        # x = torch.matmul(self.sym_norm_Adj_matrix.mul(spatial_attention), x)
        return F.relu(self.Theta(x_gcn).reshape((batch_size, num_of_timesteps, num_of_vertices, self.out_channels)).transpose(1, 2))
        # (b*t, n, f_in)->(b*t, n, f_out)->(b,t,n,f_out)->(b,n,t,f_out)
...
class DynamicAttentionLayer(nn.Module):
    ...
    def forward(self, x):
        B, N, H, T, D = x.shape
        x = x.permute(0, 2, 3, 1, 4).contiguous()  # [B,H,T,N,D]
        score = torch.matmul(x, x.transpose(-1, -2)) / math.sqrt(D)  # [B,H,T,N,D]*[B,H,T,D,N]=[B,H,T,N,N]
        score = self.dropout(F.softmax(score, dim=-1))  # [B,H,T,N,N]
        return score  # [B,H,T,N,N]


class Attention_IC(nn.Module):
    def __init__(self, norm_sc_matrix, attention_top_k):
        super(Attention_IC, self).__init__()
        ...
        self.sc_top_v = torch.softmax(sc_top_v, dim=-1).unsqueeze(-1).transpose(-2, -1)  # [C, N, 1, K]
    ...
    def forward(self, query, key, value, mask=None, dropout=None):
        ...
        # dynamic_attention = self.DAT(key)  # [B,H,T,N,N]
        # sc_mx = torch.mul(self.norm_sc_matrix, dynamic_attention)
        # sc_top = sc_mx.topk(self.k, sorted=False)
        # sc_top_v = sc_top[0]  # [B,H,T,N,K]
        # sc_top_v = torch.softmax(sc_top_v, dim=-1).unsqueeze(-1).transpose(-2, -1)  # [B,H,T,N,1,K]
        # sc_top_i = sc_top[1]  # [B,H,T,N,K]

        # key = key.permute(0, 2, 3, 1, 4)[
        #       torch.arange(B)[:, None, None, None, None],
        #       torch.arange(H)[None, :, None, None, None],
        #       torch.arange(T2)[None, None, :, None, None],
        #       sc_top_i,
        #       :]  # [B,H,T,N,K,D]
        # key = torch.matmul(sc_top_v, key).view(B, H, T2, N, D).permute(0, 3, 1, 2, 4)

        # value = value.contiguous().permute(0, 2, 3, 1, 4)[
        #         torch.arange(B)[:, None, None, None, None],
        #         torch.arange(H)[None, :, None, None, None],
        #         torch.arange(T2)[None, None, :, None, None],
        #         sc_top_i,
        #         :]  # [B,H,T,N,K,D]
        # value = torch.matmul(sc_top_v, value).view(B, H, T2, N, D).permute(0, 3, 1, 2, 4)

        key_c = key[:, self.sc_top_i[0], :, :, :].permute(0, 3, 4, 1, 2, 5)  # [B,N,K,H,T,D] => [B,H,T,N,K,D]
        key_o = torch.matmul(self.sc_top_v[0], key_c).view(B, H, T2, N, D).permute(0, 3, 1, 2, 4)

        for c in range(1, self.norm_sc_matrix.shape[0]):
            key_c = key[:, self.sc_top_i[c], :, :, :].permute(0, 3, 4, 1, 2, 5)  # [B,N,K,H,T,D] => [B,H,T,N,K,D]
            key_o += torch.matmul(self.sc_top_v[c], key_c).view(B, H, T2, N, D).permute(0, 3, 1, 2, 4)

        key = key_o / self.norm_sc_matrix.shape[0]

        scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(D)  # [B,N,H,T1,T2]

        if mask is not None:
            mask = torch.ones(T1, T2, dtype=torch.bool).to(query.device).triu(1)  # [T1,T2]
        ...
        return torch.matmul(p_attn, value), p_attn  # [B,H,N,T1,D], [B,H,N,T1,T2]


class MultiHeadAttention(nn.Module):
    def __init__(self, nb_head, d_model, norm_sc_matrix, attention_top_k, dropout=.0):
        ...
        self.norm_sc_matrix = norm_sc_matrix
        self.attn_ic = Attention_IC(self.norm_sc_matrix, attention_top_k)
    ...
    def forward(self, query, key, value, mask=None, query_multi_segment=False, key_multi_segment=False):
        ...
        x, self.attn = self.attn_ic(query, key, value, mask=mask, dropout=self.dropout)
        # x:(batch, N, h, T1, d_k)
...
class MultiHeadAttentionAwareTemporalContex_qc_kc(nn.Module):  # key causal; query causal;
    def __init__(self, nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, points_per_hour, norm_sc_matrix, attention_top_k, kernel_size=3, dropout=.0):
        """
        :param nb_head:
        :param d_model:
        :param num_of_weeks:
    ...
    def forward(self, query, key, value, mask=None, query_multi_segment=False, key_multi_segment=False):
        ...
        if query_multi_segment and key_multi_segment:
            ...
            key_list = []
            if self.w_length > 0:
                query_w, key_w = [l(x.permute(0, 3, 1, 2))[:, :, :, :-self.padding].contiguous().view(nbatches, self.h, self.d_k, N, -1).permute(0, 3, 1, 4, 2) for l, x in
                                  zip(self.conv1Ds_aware_temporal_context, (query[:, :, :self.w_length, :], key[:, :, :self.w_length, :]))]
                query_list.append(query_w)
                key_list.append(key_w)

            if self.d_length > 0:
                query_d, key_d = [l(x.permute(0, 3, 1, 2))[:, :, :, :-self.padding].contiguous().view(nbatches, self.h, self.d_k, N, -1).permute(0, 3, 1, 4, 2) for l, x in
                                  zip(self.conv1Ds_aware_temporal_context, (query[:, :, self.w_length:self.w_length + self.d_length, :], key[:, :, self.w_length:self.w_length + self.d_length, :]))]
                query_list.append(query_d)
                key_list.append(key_d)

            if self.h_length > 0:
                query_h, key_h = [l(x.permute(0, 3, 1, 2))[:, :, :, :-self.padding].contiguous().view(nbatches, self.h, self.d_k, N, -1).permute(0, 3, 1, 4, 2) for l, x in zip(self.conv1Ds_aware_temporal_context, (
                    query[:, :, self.w_length + self.d_length:self.w_length + self.d_length + self.h_length, :], key[:, :, self.w_length + self.d_length:self.w_length + self.d_length + self.h_length, :]))]
            ...
            if self.h_length > 0:
                key_h = self.conv1Ds_aware_temporal_context[1](key[:, :, self.w_length + self.d_length:self.w_length + self.d_length + self.h_length, :].permute(0, 3, 1, 2))[:, :, :, :-self.padding] \
                    .contiguous().view(nbatches, self.h, self.d_k, N, -1).permute(0, 3, 1, 4, 2)
                key_list.append(key_h)
...
class MultiHeadAttentionAwareTemporalContex_q1d_k1d(nn.Module):  # 1d conv on query, 1d conv on key
    ...
    def forward(self, query, key, value, mask=None, query_multi_segment=False, key_multi_segment=False):
        ...
        if query_multi_segment and key_multi_segment:
            ...
            key_list = []
            if self.w_length > 0:
                query_w, key_w = [l(x.permute(0, 3, 1, 2)).contiguous().view(nbatches, self.h, self.d_k, N, -1).permute(0, 3, 1, 4, 2) for l, x in
                                  zip(self.conv1Ds_aware_temporal_context, (query[:, :, :self.w_length, :], key[:, :, :self.w_length, :]))]
                query_list.append(query_w)
                key_list.append(key_w)

            if self.d_length > 0:
                query_d, key_d = [l(x.permute(0, 3, 1, 2)).contiguous().view(nbatches, self.h, self.d_k, N, -1).permute(0, 3, 1, 4, 2) for l, x in
                                  zip(self.conv1Ds_aware_temporal_context, (query[:, :, self.w_length:self.w_length + self.d_length, :], key[:, :, self.w_length:self.w_length + self.d_length, :]))]
                query_list.append(query_d)
                key_list.append(key_d)
...
class MultiHeadAttentionAwareTemporalContex_qc_k1d(nn.Module):  # query: causal conv; key 1d conv
    ...
    def forward(self, query, key, value, mask=None, query_multi_segment=False, key_multi_segment=False):
        ...
        if query_multi_segment and key_multi_segment:
            ...
            if self.d_length > 0:
                query_d = self.query_conv1Ds_aware_temporal_context(query[:, :, self.w_length:self.w_length + self.d_length, :].permute(0, 3, 1, 2))[:, :, :, :-self.causal_padding] \
                    .contiguous().view(nbatches, self.h, self.d_k, N, -1).permute(0, 3, 1, 4, 2)
            ...
            if self.h_length > 0:
                query_h = self.query_conv1Ds_aware_temporal_context(query[:, :, self.w_length + self.d_length:self.w_length + self.d_length + self.h_length, :].permute(0, 3, 1, 2))[:, :, :, :-self.causal_padding] \
                    .contiguous().view(nbatches, self.h, self.d_k, N, -1).permute(0, 3, 1, 4, 2)
...
def make_model(DEVICE, num_layers, encoder_input_size, decoder_output_size, d_model, adj_mx, sc_mx, attention_top_k, nb_head, num_of_weeks,
    ...
    norm_sc_matrix = torch.from_numpy(multichannel_norm_adj(sc_mx)).type(torch.FloatTensor).to(DEVICE)
    ...
    if ScaledSAt:  # employ spatial self attention
        position_wise_gcn = PositionWiseGCNFeedForward(spatialAttentionScaledGCN(norm_Adj_matrix, norm_sc_matrix, d_model, d_model), dropout=dropout)
    else:  # 不带attention
        position_wise_gcn = PositionWiseGCNFeedForward(spatialGCN(norm_Adj_matrix, norm_sc_matrix, d_model, d_model), dropout=dropout)
    ...
    if aware_temporal_context:  # employ temporal trend-aware attention
        attn_ss = MultiHeadAttentionAwareTemporalContex_q1d_k1d(nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, num_for_predict, norm_sc_matrix, attention_top_k, kernel_size, dropout=dropout)  # encoder的trend-aware attention用一维卷积
        attn_st = MultiHeadAttentionAwareTemporalContex_qc_k1d(nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, num_for_predict, norm_sc_matrix, attention_top_k, kernel_size, dropout=dropout)
        att_tt = MultiHeadAttentionAwareTemporalContex_qc_kc(nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, num_for_predict, norm_sc_matrix, attention_top_k, kernel_size, dropout=dropout)  # decoder的trend-aware attention用因果卷积
    else:  # employ traditional self attention
        attn_ss = MultiHeadAttention(nb_head, d_model, norm_sc_matrix, attention_top_k, dropout=dropout)
        attn_st = MultiHeadAttention(nb_head, d_model, norm_sc_matrix, attention_top_k, dropout=dropout)
        att_tt = MultiHeadAttention(nb_head, d_model, norm_sc_matrix, attention_top_k, dropout=dropout)
    ...
    if SE and TE:
        ...
        spatial_position = SpatialPositionalEncoding(d_model, num_of_vertices, dropout, GCN(norm_Adj_matrix, norm_sc_matrix, d_model, d_model), smooth_layer_num=smooth_layer_num)
