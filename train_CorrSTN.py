...
import argparse
import configparser
from model.CorrSTN import make_model
...
if config.has_option('Data', 'id_filename'):
    ...
    id_filename = None
sc_filename = data_config['sc_filename']
...
attention_top_k = int(training_config['attention_top_k'])
print('attention_top_k:', attention_top_k, flush=True)
...
folder_dir = 'MAE_%s_h%dd%dw%d_layer%d_head%d_dm%d_channel%d_dir%d_drop%.2f_%.2e_B%d_K%d_' % \
             (model_name, num_of_hours, num_of_days, num_of_weeks, num_layers, nb_head, d_model, encoder_input_size, direction, dropout, learning_rate, batch_size, attention_top_k)
...
net = make_model(DEVICE, num_layers, encoder_input_size, decoder_input_size, d_model, adj_mx, sc_mx, attention_top_k, nb_head, num_of_weeks,
                 num_of_days, num_of_hours, points_per_hour, num_for_predict,
                 dropout=dropout, aware_temporal_context=aware_temporal_context, ScaledSAt=ScaledSAt, SE=SE, TE=TE, kernel_size=kernel_size, smooth_layer_num=smooth_layer_num,
